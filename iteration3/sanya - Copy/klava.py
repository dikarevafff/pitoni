...
def get_fin(a_str, g=0):
    ...
    """
    if g == 1:
        return a_str.translate(data.fingers_map_diktor).split(',')
    if g == 0:
        return a_str.translate(data.fingers_map_QWERTY).split(',')
    if g == 2:
        return a_str.translate(data.fingers_map_vyzov).split(',')
    if g == 3:
        return a_str.translate(data.fingers_map_skoropis).split(',')
...
def get_b_bukvi(text, g=0):
    ...
    if g == 0:
        for i in text:
            if i in data.left_shift_QWERTY:
                left += 1
            if i in data.l_s_QWERTY and i == text[-1]:
                left_prob += 1
            if i in data.right_shift_QWERTY:
                right += 1
            if i in data.r_s_QWERTY and i == text[-1]:
                right_prob += 1
    if g == 1:
        for i in text:
            if i in data.left_shift_diktor:
                left += 1
            if i in data.l_s_diktor and i == text[-1]:
                left_prob += 1
            if i in data.right_shift_diktor:
                right += 1
            if i in data.r_s_diktor and i == text[-1]:
                right_prob += 1
    if g == 2:
        for i in text:
            if i in data.left_shift_vyzov:
                left += 1
            if i in data.l_s_vyzov and i == text[-1]:
                left_prob += 1
            if i in data.right_shift_vyzov:
                right += 1
            if i in data.r_s_vyzov and i == text[-1]:
                right_prob += 1
    if g == 3:
        for i in text:
            if i in data.left_shift_skoropis:
                left += 1
            if i in data.l_s_skoropis and i == text[-1]:
                left_prob += 1
            if i in data.right_shift_skoropis:
                right += 1
            if i in data.r_s_skoropis and i == text[-1]:
                right_prob += 1
...
def counter(text, g=0):
    ...
    rf5 = 0
    for i in text:
        if data.cyr[i][g] == data.lfi1:
            lf1 += 1
        if data.cyr[i][g] in data.lfi2:
            lf2 += 1
        if data.cyr[i][g] in data.lfi3:
            lf3 += 1
        if data.cyr[i][g] in data.lfi4:
            lf4 += 1
        if data.cyr[i][g] in data.lfi5:
            lf5 += 1
        if data.cyr[i][g] == data.rfi1:
            rf1 += 1
        if data.cyr[i][g] in data.rfi2:
            rf2 += 1
        if data.cyr[i][g] in data.rfi3:
            rf3 += 1
        if data.cyr[i][g] in data.rfi4:
            rf4 += 1
        if data.cyr[i][g] in data.rfi5:
            rf5 += 1
